	...
	module.declare('pm.mobi',[$global,'pm','pm.view','os'],function(require){
	    ...
	    History.prototype={
	    	...
	    	handleState:function(el,title,his,handle){
	    		...
	    		//
	    		var uri=[his||this.baseURI,search,el].join('');
		...
		MobiView.setRem=function(){
			...
			rem=parseFloat(width*MobiView.basePX/MobiView.normalWidth).toFixed(2);
			//
		...
		MobiView.initView=function(){
			...
			MobiView.history=new History({baseURI:MobiView.baseURI,el:actived,title:document.title,timestamp:MobiView.historyTimeStamp,state:state});
			//
		...
		MobiView.createDomView=function(options){
			...
			mobiLoadCallback && mobiLoadCallback.call(v);
			...
			mobiLoadedCallback && mobiLoadedCallback.call(v);
		...
		MobiView.loadContent=function(el,options){
			...
			mobiLoadCallback && mobiLoadCallback.call(nview);
			//direction
			var direction=mobis.direction||'left';
			//
			view.mobis.direction=mobis.direction || MobiView.translateDirection;
			//go back
			...
			return MobiView.doAnimation($current,$active,{
				...
				after:function(){
					MobiView.contentHandle(nview,function(){
						...
						mobiLoadedCallback && mobiLoadedCallback.call(nview);
